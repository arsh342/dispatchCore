...
const placeBid = [
  ...
  body('offered_price').isFloat({ min: 0.01 }).withMessage('Offered price must be greater than 0'),
  body('message')
...
const manageBid = [param('id').isInt({ min: 1 }).withMessage('Bid ID must be a positive integer')];
